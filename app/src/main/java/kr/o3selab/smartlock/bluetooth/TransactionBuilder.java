...
public class TransactionBuilder {
    ...
    public class Transaction {
        ...
        // Transaction common status
        private static final int STATE_NONE = 0;        // Common created
        ...
        public boolean sendTransaction() {
            ...
            if (mBuffer.length > 0) {
                ...
                Log.d(TAG, "안드->ble : " + mBuffer.toString());
                Log.d(TAG, "안드->ble : " + sb.toString());
            }
            if (mState == STATE_SETTING_FINISHED) {
                if (mBleManager != null) {
                    ...
                    if (mBleManager.getState() == BleManager.STATE_CONNECTED) {
                        // 보낼수 있는 데이터인지 체크
                        if (mBuffer.length > 0) {
                            // blemanager 에게 write 하며 buffer 송신
