...
bool MPU9250::isConnecting(void) {
    uint8_t mpu = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);
    return mpu == 0x71;
}

bool MPU9250::isAKConnecting(void) {
    uint8_t mag = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);
    return mag == 0x48;
}

bool MPU9250::isSleepMode(void) {
  uint8_t data = readByte(MPU9250_ADDRESS, PWR_MGMT_1);
  return (data & 0x40) == 0x40;
}

void MPU9250::setSleepMode(const bool enable) {

  if (isSleepMode() && enable) return; 
  
  uint8_t mask_val;
  uint8_t ori_val = readByte(MPU9250_ADDRESS, PWR_MGMT_1);

  if (isSleepMode()) {
    // mode = sleep, want no sleep
    mask_val = ori_val ^ 0x40;
  } else { 
    // mode = no sleep, want sleep
    mask_val = ori_val | 0x40;
  }

  writeByte(MPU9250_ADDRESS, PWR_MGMT_1, mask_val);
  delay(100);
}